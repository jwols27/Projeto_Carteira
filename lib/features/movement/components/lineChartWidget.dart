  ...
  void didChangeDependencies() {
    ...
    if (!loaded) {
      for (var i = 0; i < _movsStore.movs.length; i++) {
        chartData.add(SalesData(_movsStore.movsDates[i], _movsStore.movsValuesPercent[i]));
      }
    }
    loaded = true;
  }
    ...
    return Center(
        child: loaded
            ? SfCartesianChart(
                ...
                primaryYAxis: NumericAxis(title: AxisTitle(text: 'Valor(R\$)'), labelFormat: 'R\$ {value},00'),
                series: <ChartSeries>[
                    // Renders line chart
